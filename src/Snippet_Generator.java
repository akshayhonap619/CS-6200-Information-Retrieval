...
public class Snippet_Generator {

	public static LinkedHashMap<String, ArrayList<String>> getRetdocs () throws IOException 
	{
	...
	{ 
	 ...
	 return docfreq;
	   ...
	   return frequencyData.get(word); // Auto-unboxed
		...
		{
			LinkedHashMap<String, ArrayList<String>> rd = getRetdocs();
			System.out.println("-------");
			System.out.println(rd.get("1"));
			
			QueryReader qr = new QueryReader();
			
		LinkedHashMap<String, String> myqueries	= qr.getQueries();
			
			ArrayList<String> mj = new ArrayList<>();
			
			
			for(int h = 1 ; h<= 5 ; h++)
			{
			mj = rd.get(String.valueOf(h));
			String basepath = new File("ProjectCorpus").getAbsolutePath();
			...
			{
				// for 5 files
				BufferedReader br = new BufferedReader(new FileReader(basepath+"\\"+mj.get(i)+".txt" ));
				  ...
				  String buffer ="", filetext = "";
				   	while((buffer = br.readLine())!=null)
			   	 {
				   	...
				   	filetext = filtertext(filetext);
				   	
				   	filetext = getSummary(filetext, myqueries.get("1"));
